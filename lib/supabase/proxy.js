...
export async function updateSession(request){
 ...
 const publicPath=path==='/'||path.startsWith('/login')||path.startsWith('/registro')||path.startsWith('/recuperar')||path.startsWith('/auth')
 if(!data?.claims&&!publicPath){const url=request.nextUrl.clone();url.pathname='/login';return NextResponse.redirect(url)}
 return response
}
